...
pub fn build_cli<'a>(app: &str, home: &'a str, version: &'a str) -> App<'a, 'a> {
  App::new(app)
    ...
    //   .long("source")
    //   .value_name("FILE")
    //   .help("Path to source data input file. Input (-i) will supercede this option")
    //   .takes_value(true))
    .arg(Arg::with_name("template")
       .short("t")
       .long("template")
       .value_name("FILE")
       .help("Path to template input file")
       .takes_value(true))
    ...
    .subcommand(SubCommand::with_name("json")
      //.about("Data options (raw, json, url, toml, yaml)")
      .about("Data options (json only at moment)"))
      /*
      .subcommand(SubCommand::with_name("raw")
         .about("Data is a raw string using url or json format")
         .arg_from_usage("[data] 'Data string'")
         .arg_from_usage("[format] 'Format: JSON or URL (key=value&key=value pairs). Default is URL'")
         .arg_from_usage("[output] 'Output Path'"))
      .subcommand(SubCommand::with_name("json")
         .about("Data is in json format")
         .arg_from_usage("[path] 'Path to JSON file'")
         .arg_from_usage("[output] 'Output Path'"))
      .subcommand(SubCommand::with_name("url")
         .about("Data is located at a given URL and returned in JSON format")
         .arg_from_usage("[url] 'URL of JSON data'")
         .arg_from_usage("[output] 'Output Path'"))
      .subcommand(SubCommand::with_name("toml")
         .about("Data is in toml format")
         .arg_from_usage("[path] 'Path to toml file'")
         .arg_from_usage("[output] 'Output Path'"))
      .subcommand(SubCommand::with_name("yaml")
         .about("Data is in yaml format")
         .arg_from_usage("[path] 'Path to yaml file'")
         .arg_from_usage("[output] 'Output Path'")))
      */
}
